...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ user, journals, onUpdateJournal, users }) => {
  ...
  const studentSummaries = useMemo(() => {
    return myStudents.map(student => {
      ...
      const approvedJournals = studentJournals.filter(j => j.status === 'Approved').length;
      const pendingJournals = studentJournals.filter(j => j.status === 'Pending').length;

      let dominantMood: Mood | 'N/A' = 'N/A';
      if (totalJournals > 0) {
        const moodCounts = studentJournals.reduce((acc, entry) => {
          acc[entry.mood] = (acc[entry.mood] || 0) + 1;
          return acc;
        }, {} as Record<Mood, number>);
        
        dominantMood = Object.keys(moodCounts).reduce((a, b) => moodCounts[a as Mood] > moodCounts[b as Mood] ? a : b) as Mood;
      }
  ...
  const handleViewStudentJournals = (studentId: string) => {
  ...
  const handleGenerateComment = async () => {
    if (!selectedJournal) {
      displayNotification('Jurnal tidak dipilih.', 'error');
      return;
    }
    const student = getStudent(selectedJournal.studentId);
    if (!student) {
      displayNotification('Data siswa tidak ditemukan.', 'error');
      return;
    }
    setIsGeneratingComment(true);
    setTeacherComment('Membuat saran komentar...');
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const prompt = `Anda adalah asisten guru yang berempati dan membantu memberikan umpan balik yang positif. Buatlah komentar singkat (1-2 kalimat) yang memberi semangat untuk siswa bernama ${student.name} berdasarkan entri jurnalnya. Jangan ulangi detail jurnal, fokus pada memberikan dorongan atau validasi perasaan. Komentar harus dalam Bahasa Indonesia.\n\nDetail Jurnal:\n- Aktivitas: ${selectedJournal.activity}\n- Perasaan (Mood): ${selectedJournal.mood}\n- Refleksi: ${selectedJournal.reflection}`;
      
      const response = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
        contents: prompt,
        config: {
          systemInstruction: 'Anda adalah asisten guru yang berempati dan membantu memberikan umpan balik yang positif dan membangun dalam Bahasa Indonesia.'
        }
      });

      setTeacherComment(response.text);
    } catch (error) {
      console.error("Error generating comment:", error);
      displayNotification('Gagal membuat saran komentar. Coba lagi.', 'error');
      setTeacherComment('');
    } finally {
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="p-6 space-y-4 overflow-y-auto">
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <label className="block text-sm font-medium">Catatan / Umpan Balik Anda</label>
                            <button
                                type="button"
                                onClick={handleGenerateComment}
                                disabled={isGeneratingComment}
                                className="text-xs px-2 py-1 bg-primary-100 text-primary-700 dark:bg-primary-900/50 dark:text-primary-300 rounded-md hover:bg-primary-200 dark:hover:bg-primary-800 transition-colors disabled:opacity-50 disabled:cursor-wait"
                            >
                                {isGeneratingComment ? (
                                    <><i className="fas fa-spinner fa-spin mr-1"></i> Membuat...</>
                                ) : (
                                    <><i className="fas fa-wand-magic-sparkles mr-1"></i>Buat Saran</>
                                )}
                            </button>
                        </div>
      ...
      <Card>
        <h3 className="text-xl font-semibold mb-4"><i className="fas fa-users-viewfinder mr-2 text-primary-500"></i>Ringkasan per Siswa</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {studentSummaries.map(summary => {
            const moodDetails = summary.dominantMood !== 'N/A' ? getMoodDetails(summary.dominantMood) : null;
            return (
              <div key={summary.id} className="p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 flex flex-col justify-between">
                <div>
                  <div className="flex items-center mb-3">
                    <img src={summary.avatar} alt={summary.name} className="w-12 h-12 rounded-full mr-3" />
                    <div>
                      <p className="font-semibold text-gray-800 dark:text-white">{summary.name}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{summary.class}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center text-sm mb-3">
                    <div>
                      <p className="font-bold text-lg">{summary.totalJournals}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">Total</p>
                    </div>
                    <div>
                      <p className="font-bold text-lg text-green-500">{summary.approvedJournals}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">Disetujui</p>
                    </div>
                    <div>
                      <p className="font-bold text-lg text-yellow-500">{summary.pendingJournals}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">Pending</p>
                    </div>
                  </div>
                  <div className="text-sm">
                    <span className="text-gray-600 dark:text-gray-300">Mood Dominan: </span>
                    {moodDetails ? (
                       <span className={`font-semibold ${moodDetails.color}`}>
                         <i className={`fas ${moodDetails.icon} mr-1.5`}></i>
                ...
                <button 
                  onClick={() => handleViewStudentJournals(summary.id)} 
                  className="mt-4 w-full text-primary-600 dark:text-primary-400 bg-primary-100 dark:bg-primary-900/50 hover:bg-primary-200 dark:hover:bg-primary-800 font-medium rounded-lg text-sm px-5 py-2 text-center transition-colors"
                >
                  Lihat Jurnal
                </button>
              </div>
