...
const ParentDashboard: React.FC<ParentDashboardProps> = ({ user, journals, users }) => {
  ...
  const totalPages = Math.ceil(visibleJournals.length / ITEMS_PER_PAGE);

  const paginatedVisibleJournals = useMemo(() => {
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    return visibleJournals.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  }, [visibleJournals, currentPage]);
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <div className="space-y-4 min-h-[450px]">
           ...
           ) : paginatedVisibleJournals.length === 0 ? (
                <div className="text-center py-10 text-gray-500 dark:text-gray-400">
           ...
           ) : paginatedVisibleJournals.map(entry => (
